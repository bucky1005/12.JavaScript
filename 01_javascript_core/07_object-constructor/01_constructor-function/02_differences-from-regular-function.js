/* 02. 02_differences-from-regular-function(일반 함수와의 차이점들) */
...
function Dog(name, species) {
    ...
    if(!new.target) {                           // new 없이 함수 호출 시 true가 되게 하는 구문
        return new Dog(name, species);          // new 연산자와 함께 생셩자 함수를 재귀 호출하여 생성된 인스턴스를 반환한다.
    }
